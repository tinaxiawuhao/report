...
import com.ihrm.report.excelModel.util.FWrite;
...
//        });
     ...
     * 获取当前行指定列数据
     *
     * @param headers {@link List}{@link List< com.ihrm.report.excelModel.excel.entity.Cell >} 来自 {@link ExcelReader#headers()}
     * @return {@link com.ihrm.report.excelModel.excel.entity.Row}{@link com.ihrm.report.excelModel.excel.entity.Row<int:Header对象中的index字段值, String:单元格内容>}
     */
    public com.ihrm.report.excelModel.excel.entity.Row rowObject(final List<com.ihrm.report.excelModel.excel.entity.Cell> headers) {
//        final LinkedHashMap<String, String> map = new LinkedHashMap<>();
        final com.ihrm.report.excelModel.excel.entity.Row row = com.ihrm.report.excelModel.excel.entity.Row.build();
        headers.forEach(header -> row.addCell(header.index(),
                com.ihrm.report.excelModel.excel.entity.Cell.builder()
...
//                        .sindex()
     ...
     */
    public JSONObject rowJSONObject(final List<com.ihrm.report.excelModel.excel.entity.Cell> headers) {
//        final LinkedHashMap<String, String> map = new LinkedHashMap<>();
    ...
    public static void main(String[] args) {
        {
            ...
            read.accept(FPath.of("src/test/files/excelModel/test.xls").file());
            read.accept(FPath.of("src/test/files/excelModel/test.xlsx").file());
        }
        {
            final Consumer<File> read = (file) -> {
            ...
            read.accept(FPath.of("src/test/files/excelModel/地区划分.xls").file());
            read.accept(FPath.of("src/test/files/excelModel/地区划分.xlsx").file());
        }
        {
            final Consumer<File> read = (file) -> {
                @Cleanup final ExcelReader reader = ExcelReader.of(file).sheet(0); // 读取第 1 个 sheet
//                @Cleanup final ExcelReader reader = ExcelReader.of(file, true, "111111").sheet(0); // 读取第 1 个 sheet
                final List<com.ihrm.report.excelModel.excel.entity.Cell> header = reader.row(Rownum.of(1)).headers();
                final List<com.ihrm.report.excelModel.excel.entity.Row> body = new ArrayList<>();
                while (reader.hasNext()) {
                    body.add(reader.rowObject(header));
                }
                System.out.println(FWrite.of(String.format("src/test/files/temp/%s.json", file.getName()))
                        .writeJson(
                                com.ihrm.report.excelModel.excel.entity.Table.builder()
                                        .header(header)
                                        .body(body)
                                        .build()
                        )
            ...
            read.accept(FPath.of("src/test/files/excelModel/联系人-111111.xls").file());
            read.accept(FPath.of("src/test/files/excelModel/联系人-111111.xlsx").file());
        }
    }
}
